...
const FULL_BOARDS_IN_SECTION = Math.floor( BEAMS_REQUIRED_EVERY_INCHES / BOARD_LENGTH )
...
console.log({
    ...
    BOARD_LENGTH,
    WASTE_MULTIPLIER,
    BEAMS_REQUIRED_EVERY_INCHES,
    FULL_BOARD_SECTION_SIZE
});
...
function getRequiredBeamsInLength( inches:number ){
    ...
    const wallLengthOverMinRequired = getWallLengthOverMinimumRequiredBeforeBeam( inches );
    const wallLengthPlusBeam = BEAMS_REQUIRED_EVERY_INCHES + BEAM_WIDTH;
    const requiredBeams = Math.ceil( wallLengthOverMinRequired / wallLengthPlusBeam );
...
function isBeamRequired( inches:number ): number {
    ...
    const wallLengthOverMinRequired = Math.max(inches - BEAMS_REQUIRED_EVERY_INCHES, 0);
...
function getFullSections( inches:number, beams:number ){
    ...
    const inchesReducedPerSection = BEAMS_REQUIRED_EVERY_INCHES - FULL_BOARD_SECTION_SIZE;

    // how big is the last section if all beams are at BEAMS_REQUIRED_EVERY_INCHES
    const lastSectionSize = inches - ( beams * ( BEAMS_REQUIRED_EVERY_INCHES + BEAM_WIDTH ));

    // how many inches of boards can we add to the last section before it will add an additional beam to the structure
    const remainingBeforeNewBeam = BEAMS_REQUIRED_EVERY_INCHES - lastSectionSize;

    // how many complete portions of the inchesReducedPerSection can we move to the last section
    let fullSections = Math.floor( remainingBeforeNewBeam / inchesReducedPerSection );
    ...
    console.log({
        ...
        inches,
        beams,
        inchesReducedPerSection,
        lastSectionSize,
        remainingBeforeNewBeam,
        fullSections
    });
...
function getLastSectionSize( inches:number, beams:number ){
    ...
    const lastSectionSize = inches - ( beams * BEAM_WIDTH ) - ( fullSections * FULL_BOARD_SECTION_SIZE );
